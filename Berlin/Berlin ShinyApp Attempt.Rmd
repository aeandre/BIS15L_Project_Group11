...
server <- function(input, output, session) { 
  output$plot <- renderPlot({
  ggplot(microbiome_diversity, aes_string(x = input$x, y = input$y, fill= "group")) + geom_col(position = "dodge") + theme(axis.text.x = element_text(hjust = 5)) + theme(plot.title = element_text(size = rel(1.5), hjust = 5))+ scale_fill_brewer(palette = "Set1")
  })
  session$onSessionEnded(stopApp)
  }
shinyApp(ui, server)
