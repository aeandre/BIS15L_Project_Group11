---
title: "Venn Diagram"
author: "Allison Andre"
date: "2/24/2021"
output: html_document
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
```

## Load the libraries
```{r message=FALSE, warning=FALSE}
library(tidyverse)
library(janitor)
library(here)
library(naniar)
library(ggvenn)
library(VennDiagram)
``` 

## Load the Data
```{r}
tidier_microbiome <- readr::read_csv("/Users/allyandre/Desktop/BIS15L_Project_Group11/tidy_data/tidiermicrobiome.csv")
tidier_microbiome
```
```{r}
asd_tidier <- readr::read_csv("/Users/allyandre/Desktop/BIS15L_Project_Group11/tidy_data/groups_tidier/asd_group_tidier.csv")
asd_tidier
```

```{r}
td_tidier <- readr::read_csv("/Users/allyandre/Desktop/BIS15L_Project_Group11/tidy_data/groups_tidier/td_group_tidier.csv")
td_tidier
```

## Prepare data for Venn diagram
```{r}
td_venn <-
  td_tidier %>% 
  rename(genus_td = "genus") %>% 
  filter(abundance != 0) %>% 
  select(genus_td, patient_id)
```

```{r}
asd_venn <-
  asd_tidier %>% 
  rename(genus_asd = "genus") %>% 
  filter(abundance != 0)
```

```{r}
both_venn <- full_join(td_venn, asd_venn, by = "patient_id")
both_venn
```

```{r}
both_venn$genus_td <- as.factor(both_venn$genus_td)
both_venn$genus_asd <- as.factor(both_venn$genus_asd)
```

```{r}
both_venn2 <- 
  both_venn %>% 
  filter(genus_td != "NA" | genus_asd != "NA")
```


## Build Venn Diagram

```{r}
venn_data <- list(
  "TD Genera" = both_venn2$genus_td,
  "ASD Genera" = both_venn2$genus_asd
)
```



```{r}
ggvenn(venn_data, fill_color = c("yellow", "turquoise1"))
```

## Try to build a COOLER venn diagram

```{r}
anyNA(venn_data)
```


```{r}
venn.diagram(venn_data, category.names = c("TD Genera", "ASD Genera"), filename = 'cool_venn.png', output = T, imagetype = "png", height = 480. width = 480, resolution = 300, compression = "lzw")
```


## Bargraphs of number of genera unique to each group

```{r}
unique_names <- c("TD group", "ASD Group")
unique_counts <- c(196, 33)
unique_genera <- data_frame(unique_names, unique_counts)
unique_genera
```

```{r}
unique_genera %>% 
  ggplot(aes(x = unique_names, y = unique_counts, fill = unique_names)) +
  geom_col(color = "black") + 
  scale_fill_manual(values = c("turquoise1", "yellow")) +
  theme_linedraw() +
  labs(title = "Number of Unique Genera Per Group",
       x = NULL,
       y = "Unique Genera") +
  theme(plot.title = element_text(size = rel(1.5), hjust = 0.5)) 
```


